...
There were no Nulls or data integrity issues with ride_id, rideable_type, member_casual columns. Verified with a simple WHERE IS NULL Select statement

Data revealed a few data issues:
	
	1.) There exist many rides where the bike was checked out and immediately checked back in to the same station.
	2.) Many rides were rented for absurd amounts of time and should be removed.
	3.) A few negative rent_times exist and should also be removed.
	4.) Many Nulls exist for both return and start locations. These will not be included in analysis either.    */


--Data Preparing
-- Filtering out of station Nulls, immediate rent cancelations, and negative time intervals. 
...
/*
Various additional columns were thought to be added for aiding the analysis with Pandas/Tableau:
	    ...
	    With pythagorean formula and a google search for the ratio of degrees longitude/latitude to miles I used:
             -> sqrt(start_lat - end_lat)*69)^2 + ((start_lng-end_lng)*54.6)^2) to produce this column. 
			
	    The timestamps for "started_at" and "ended_at" are being used to generate multiple columns:
...
SELECT ride_id, rideable_type, start_station_name, end_station_name, start_lat, start_lng, end_lat, end_lng, member_casual,
      ...
      (CASE WHEN EXTRACT(DOW FROM started_at) = 0 THEN 'Sunday' 
		...
		WHEN EXTRACT(DOW FROM started_at) = 3 THEN 'Wednesday'
	 	...
	 	WHEN EXTRACT(DOW FROM started_at) = 6 THEN 'Saturday'  END) AS weekday
	...
	SELECT member_casual,
		(CASE 
		 	WHEN weekday = 'Saturday' OR weekday = 'Sunday' THEN 'weekend' 
		        ELSE 'weekday' END) AS Weekend_weekday
	FROM RiderDataCln )
...
--
SELECT *, sum(d.cnt) OVER(partition by member_casual) as agg 
FROM(
